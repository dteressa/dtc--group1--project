...
The causal structure of this analysis is shown in the diagram above. A driver’s blood alcohol content (BAC) is the running variable that determines whether they cross the 0.08 legal cutoff. Crossing this threshold leads to harsher punishment, which is our treatment of interest. This treatment then affects the probability of future recidivism, which is the outcome we care about.

There are also background factors that affect both BAC and recidivism. These include observable characteristics like gender, race, and age, as well as unobservable factors like risk tolerance or drinking behavior. Drivers with higher risk tolerance may both drive with elevated BAC and be more prone to reoffending. A simple comparison of drivers above and below the cutoff would therefore not identify a causal effect.

The idea of the RDD is that near the 0.08 cutoff, treatment changes sharply, but everything else changes smoothly. This means that drivers just above and just below the cutoff should be very similar in both observed and unobserved characteristics. So any jump in recidivism at the cutoff can be attributed to the change in punishment.

We also check whether these characteristics are smooth at the cutoff as a way to assess the validity of the RDD assumptions.
...
dagify(
  ...
  theme_dag() +
  labs(title = "Causal diagram: DWI harsher punishment and recidivism")
```
